...
/** Technical Redis adapter for the Identity-owned OTP challenge persistence port. */
export class RedisOtpChallengeRepository implements IOtpChallengeRepository {
  ...
  private parseChallenge(raw: string): OtpChallenge {
    ...
    return {
      ...
      consumedAt: parsed.consumedAt === null ? null : parseDate(parsed.consumedAt),
      invalidatedAt: parsed.invalidatedAt === null ? null : parseDate(parsed.invalidatedAt),
  ...
  private validateCreateInput(input: CreateOtpChallengeInput, guard: OtpChallengeRateLimitGuard): void {
    ...
    if (!Number.isInteger(input.maxAttempts) || input.maxAttempts <= 0) throw new RangeError('maxAttempts must be a positive integer');
    if (!Number.isInteger(guard.maxChallenges) || guard.maxChallenges <= 0) throw new RangeError('maxChallenges must be a positive integer');
    if (guard.windowStart.getTime() > guard.now.getTime()) throw new RangeError('rate-limit windowStart must not be after now');
    if (input.expiresAt.getTime() <= guard.now.getTime()) throw new RangeError('OTP challenge expiresAt must be after now');
  }
}
...
function challengeKey(id: number): string { return `${OTP_NAMESPACE}challenge:${id}`; }
function publicKey(publicId: string): string { return `${OTP_NAMESPACE}public:${publicId}`; }
function latestByPhoneKey(phoneNumber: string): string { return `${OTP_NAMESPACE}phone:${encodePhoneKey(phoneNumber)}:latest`; }
function rateKey(phoneNumber: string): string { return `${OTP_NAMESPACE}rate:${encodePhoneKey(phoneNumber)}`; }
