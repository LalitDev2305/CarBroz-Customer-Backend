...
export class MinIOStorageProvider {
    ...
    async getClient() {
        if (this.client || this.isMockMode)
            return this.client;
        const endPoint = await this.configProvider.get('MINIO_ENDPOINT');
        const portStr = await this.configProvider.get('MINIO_PORT');
        const accessKey = await this.configProvider.get('MINIO_ACCESS_KEY');
        const secretKey = await this.configProvider.get('MINIO_SECRET_KEY');
    ...
    async uploadFile(bucket, objectName, buffer, mimeType) {
        ...
        if (this.isMockMode) {
            ...
            // Create subdirectories if objectName has slashes
        ...
        const endPoint = await this.configProvider.get('MINIO_ENDPOINT');
        const port = await this.configProvider.get('MINIO_PORT');
        const ssl = await this.configProvider.get('MINIO_USE_SSL') === 'true' ? 'https' : 'http';
...
//# sourceMappingURL=MinIOStorageProvider.js.map