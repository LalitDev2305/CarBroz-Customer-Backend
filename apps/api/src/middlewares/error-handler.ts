...
import { AppError } from '@carbroz/common';
...
export const globalErrorHandler = (error: FastifyError, request: FastifyRequest, reply: FastifyReply) => {
  ...
  if (error instanceof AppError) {
    ...
    return reply.status(error.statusCode).send(
      ResponseHelper.error(error.message, error.errorCode, traceId)
    );
  }
  ...
  if (error.validation) {
    ...
    return reply.status(400).send(
      ResponseHelper.error(error.message, 'VALIDATION_ERROR', traceId)
    );
  }
  ...
  return reply.status(500).send(
    ResponseHelper.error(message, 'INTERNAL_SERVER_ERROR', traceId)
  );
};
