...
import { GuestLoginUseCase, LogoutUseCase, RefreshTokenUseCase, SendOtpUseCase, VerifyOtpUseCase } from '@carbroz/domain-identity';
import { JwtConfig } from '../../bootstrap/config/runtime-config.js';
import { ResponseHelper } from '../response/ResponseHelper.js';
import { GuestLoginSchema, LogoutSchema, RefreshTokenSchema, SendOtpSchema, VerifyOtpSchema } from './dto/auth.dto.js';

/** HTTP transport adapter for Identity authentication use cases. */
export class AuthController {
  public async guestLogin(request: FastifyRequest, reply: FastifyReply) {
    ...
    } as any, { expiresIn: JwtConfig.accessExpiration });
  ...
  public async logoutAll(request: FastifyRequest, reply: FastifyReply) {
    ...
    if (userId) await logoutUseCase.execute({ logoutAll: true, userId });
