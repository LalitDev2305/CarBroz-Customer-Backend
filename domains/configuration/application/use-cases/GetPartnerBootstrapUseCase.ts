...
export class GetPartnerBootstrapUseCase {
  ...
  async execute(input: PartnerBootstrapRequest): Promise<PartnerBootstrapSnapshot> {
    const document = await this.configProvider.get<PartnerBootstrapDocument>(PARTNER_BOOTSTRAP_CONFIG_KEY, DEFAULT_PARTNER_BOOTSTRAP_DOCUMENT);
    ...
    return {
      ...
      startup: { authenticated: input.authenticated, nextScreen: input.authenticated ? document.startup.authenticated : document.startup.guest },
    };
  }

  private assertDocument(document: PartnerBootstrapDocument): void {
    if (!document || typeof document !== 'object' || !document.version?.trim()) throw new DomainError('Invalid Partner bootstrap configuration document');
  ...
  private assertUpdate(update: PartnerPlatformUpdateConfig | undefined, platform: string): asserts update is PartnerPlatformUpdateConfig {
    ...
    if (compareApplicationVersions(update.latestVersion, update.minimumVersion) < 0) throw new DomainError(`Invalid Partner update configuration for ${platform}: latestVersion is below minimumVersion`);
  }

  private assertScreen(screen: PartnerStartupScreenConfig | undefined, state: string): void {
    if (!screen || !screen.screenId?.trim() || !screen.templateId?.trim() || !screen.templateType?.trim()) throw new DomainError(`Invalid Partner ${state} startup screen configuration`);
    if (!screen.endpoint.startsWith('/') || screen.endpoint.startsWith('//') || screen.endpoint.includes('://')) throw new DomainError(`Invalid Partner ${state} startup endpoint`);
  }
}
...
function parseApplicationVersionCore(version: string): number[] {
  ...
  if (!match) throw new DomainError(`Invalid application version: ${version}`);
