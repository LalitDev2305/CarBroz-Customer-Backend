...
export class CorporateCreditAccountingService implements ICorporateCreditAccounting {
  ...
  async recordCreditGrant(input: CorporateCreditGrantInput): Promise<void> {
    await this.creditLedgerRepo.create(new CorporateCreditLedger({
  ...
  async recordCreditAdjustment(input: CorporateCreditAdjustmentInput): Promise<void> {
    await this.creditLedgerRepo.create(new CorporateCreditLedger({
      corporateAccountId: input.corporateAccountId,
  ...
  async recordBookingDebit(input: CorporateBookingDebitInput): Promise<void> {
    if (input.amountPaise <= 0n) throw new Error('Corporate booking debit must be positive');
    const account = await this.corporateAccountRepo.findById(input.corporateAccountId);
    if (!account) throw new Error('Corporate account not found');
    if (account.utilisedCreditPaise + input.amountPaise > account.creditLimitPaise) {
      throw new Error('Corporate account credit limit exceeded');
    }
    ...
    await this.creditLedgerRepo.create(new CorporateCreditLedger({
      ...
      amountPaise: input.amountPaise,
      balanceAfterPaise: updated.creditLimitPaise - updated.utilisedCreditPaise,
      referenceNotes: input.referenceNotes ?? `Booking ID ${input.bookingId} credit debit`,
    }));
  }

  async recordPaymentCredit(input: CorporatePaymentCreditInput): Promise<void> {
    if (input.amountPaise <= 0n) throw new Error('Corporate payment credit must be positive');
    const account = await this.corporateAccountRepo.findById(input.corporateAccountId);
    if (!account) throw new Error('Corporate account not found');
    if (input.amountPaise > account.utilisedCreditPaise) {
      throw new Error('Corporate payment credit exceeds utilised credit');
    }
