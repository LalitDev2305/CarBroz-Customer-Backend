...
export class Booking {
  ...
  constructor(props: BookingProps) {
    if (!props.customerId)
      throw new DomainError("Booking must belong to a customer");
    if (!props.vehicleId) throw new DomainError("Booking requires a vehicle");
    if (!props.addressId) throw new DomainError("Booking requires an address");
    if (!props.serviceId) throw new DomainError("Booking requires a service");
  ...
  confirm(actorId: number): void {
    if (this.status !== "CREATED") {
      throw new DomainError(`Cannot confirm booking in status ${this.status}`);
    }
    if (this.expiryAt && systemClock.now() > this.expiryAt) {
      this.transitionTo("EXPIRED", actorId, "Slot reservation expired");
      throw new DomainError("Cannot confirm an expired booking slot");
    }
    this.expiryAt = null;
  ...
  assignPartner(partnerId: number, actorId: number): void {
    if (this.status !== "CONFIRMED" && this.status !== "ASSIGNED") {
      throw new DomainError(
        `Cannot assign partner to booking in status ${this.status}`,
      );
    }
  ...
  startService(actorId: number): void {
    if (this.status !== "ASSIGNED") {
      throw new DomainError(
        `Cannot start service for booking in status ${this.status}`,
      );
    }
  ...
  completeService(actorId: number): void {
    if (this.status !== "IN_PROGRESS") {
      throw new DomainError(
        `Cannot complete service for booking in status ${this.status}`,
      );
    }
  ...
  cancel(actorId: number, reason: string): void {
    if (
      ...
      throw new DomainError(`Cannot cancel booking in status ${this.status}`);
    }
