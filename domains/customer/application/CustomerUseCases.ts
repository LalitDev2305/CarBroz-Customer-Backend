...
export class GetCustomerProfileUseCase implements IUseCase<
...
> {
  ...
  async execute(request: GetCustomerProfileInput): Promise<CustomerProfile> {
    ...
    const existing = await this.customerProfileRepository.findByUserId(
      request.data.userId,
    );
...
export class UpdateCustomerProfileUseCase implements IUseCase<
...
> {
  ...
  async execute(request: UpdateCustomerProfileInput): Promise<CustomerProfile> {
    ...
    if (request.data.dateOfBirth !== undefined)
      profile.dateOfBirth = request.data.dateOfBirth;
    if (request.data.gender !== undefined) profile.gender = request.data.gender;
    if (request.data.marketingOptIn !== undefined)
      profile.marketingOptIn = request.data.marketingOptIn;
    profile.updatedAt = systemClock.now();
...
export type AddressAction =
  "ADD" | "UPDATE" | "DELETE" | "GET_ALL" | "GET_DEFAULT";
...
export class ManageAddressUseCase implements IUseCase<
  ManageAddressInput,
  ManageAddressResult
> {
  ...
  async execute(request: ManageAddressInput): Promise<ManageAddressResult> {
    ...
    switch (action) {
      ...
      case "ADD":
        if (!payload)
          throw new DomainError(
            "BAD_REQUEST: Payload required for adding address",
          );
...
export class ExtractCustomerDataUseCase implements IUseCase<
...
> {
  ...
  /** Executes this application operation through its declared ports and domain invariants. */
  async execute(
    request: ExtractCustomerDataInput,
  ): Promise<ExtractedCustomerData> {
    if (!canAccessCustomer(request.context, request.data.userId)) {
      throw new DomainError(
        "FORBIDDEN: You do not have permission to extract this data",
      );
    }
    ...
    return {
      userId,
      extractedAt: systemClock.now().toISOString(),
      profile: profile
        ? {
