...
import { type IKycDocumentRepository, type NewKycDocument } from '../../domain/repositories/IKycDocumentRepository.js';
import { type KycPersistenceClient, type KycPersistenceRecord } from '../persistence/KycPersistenceClient.js';
...
export class PrismaKycDocumentRepository implements IKycDocumentRepository {
  ...
  async create(document: NewKycDocument): Promise<KycDocument> {
    const entity = await this.client.kycDocument.create({
      data: {
        ...
        ...(document.rejectionReason !== undefined ? { rejectionReason: document.rejectionReason } : {}),
        uploadedById: document.uploadedById,
        ...(document.verifiedById !== undefined ? { verifiedById: document.verifiedById } : {}),
      },
    });
  ...
  async updateStatus(
    ...
    const entity = await this.client.kycDocument.update({
      where: { id },
      data: { status, verifiedById, ...(rejectionReason !== undefined ? { rejectionReason } : {}) },
    });
    return this.mapToDomain(entity);
  }
