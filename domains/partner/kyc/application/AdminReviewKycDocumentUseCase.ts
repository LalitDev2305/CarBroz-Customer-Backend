...
export class AdminReviewKycDocumentUseCase implements IUseCase<
  AdminReviewKycDocumentInput,
  KycDocument
> {
  constructor(
  ...
  async execute({
    context,
    data,
  }: AdminReviewKycDocumentInput): Promise<KycDocument> {
    ...
    if (
      ...
      throw new DomainError("UNAUTHORIZED: Admin must be logged in");
    }
    const status =
      data.action === "APPROVE"
        ? KycDocumentStatus.APPROVED
        : KycDocumentStatus.REJECTED;
    if (status === KycDocumentStatus.REJECTED && !data.reason)
    ...
    if (!document) throw new DomainError("NOT_FOUND: KYC document not found");
    ...
    if (data.action === "APPROVE") {
      const allDocs = await this.kycDocumentRepository.findByPartnerId(
        document.partnerId,
      );
      const hasPending = allDocs.some(
        (candidate) => candidate.status === KycDocumentStatus.PENDING,
      );
      if (!hasPending) {
        const partner: Partner | null = await this.partnerRepository.findById(
          document.partnerId,
        );
        if (partner) {
