...
export interface GetPartnerProfileInput {
...
export class GetPartnerProfileUseCase implements IUseCase<
  GetPartnerProfileInput,
...
> {
  ...
  async execute({
    context,
  }: GetPartnerProfileInput): Promise<PartnerProfileResult> {
    ...
    if (!primaryMembership) throw new DomainError("Partner profile not found");
    const partner = await this.partnerRepository.findById(
      primaryMembership.partnerId,
    );
