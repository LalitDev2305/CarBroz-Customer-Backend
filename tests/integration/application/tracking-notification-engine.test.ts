...
describe('Phase 18 — Tracking & Notification Engine Use Cases', () => {
  ...
  it('should start tracking session and compute initial ETA', async () => {
    const useCase = new StartTrackingSessionUseCase(Object.assign(mockBookingRepo, { findByBookingId: async (_bookingId: number) => null }), mockTrackingRepo);

    const session = await useCase.execute({
      bookingId: dummyBooking.id,
      partnerId: dummyBooking.partnerId,
      customerId: dummyBooking.customerId,
      latitude: 12.9716,
      longitude: 77.5946,
    });
  ...
  it('should update location ping and recalculate ETA', async () => {
    const startUseCase = new StartTrackingSessionUseCase(Object.assign(mockBookingRepo, { findByBookingId: async (_bookingId: number) => null }), mockTrackingRepo);
